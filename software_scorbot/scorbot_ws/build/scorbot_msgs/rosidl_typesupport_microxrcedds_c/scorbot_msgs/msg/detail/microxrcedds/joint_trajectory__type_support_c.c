...
#include "scorbot_msgs/msg/detail/joint_trajectory__rosidl_typesupport_microxrcedds_c.h"
...
typedef scorbot_msgs__msg__JointTrajectory _JointTrajectory__ros_msg_type;
  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: points
  {
    ...
    rv = ucdr_serialize_sequence_int32_t(cdr, ros_message->points.data, size);
  }
  ...
  void * untyped_ros_message)
{
  ...
  {
    uint32_t size;
    const size_t capacity = ros_message->points.capacity;
    rv = ucdr_deserialize_sequence_int32_t(cdr, ros_message->points.data, capacity, &size);
    if (rv) {
      ros_message->points.size = size;
    } else if(size > capacity){
      cdr->error = false;
      cdr->last_data_size = 1;
      ros_message->points.size = 0;
...
size_t get_serialized_size_scorbot_msgs__msg__JointTrajectory(
  const void * untyped_ros_message,
  size_t current_alignment)
{
  if (!untyped_ros_message) {
  ...
  // Member: points
  {
    size_t sequence_size = ros_message->points.size;
    current_alignment += ucdr_alignment(current_alignment, MICROXRCEDDS_PADDING) + MICROXRCEDDS_PADDING;

    if (0 < sequence_size) {
      size_t item_size = sizeof(ros_message->points.data[0]);
...
{
  return (uint32_t)(
    get_serialized_size_scorbot_msgs__msg__JointTrajectory(
      untyped_ros_message, 0));
}

ROSIDL_TYPESUPPORT_MICROXRCEDDS_C_PUBLIC_scorbot_msgs
size_t max_serialized_size_scorbot_msgs__msg__JointTrajectory(
  bool * full_bounded,
  size_t current_alignment)
{
...
{
  bool full_bounded;
  return max_serialized_size_scorbot_msgs__msg__JointTrajectory(&full_bounded, 0);
}
...
const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, scorbot_msgs, msg, JointTrajectory)() {
